...
- 用于方法调用。在 Java 源文件被编译到字节码文件中时，所有的变量和方法引用都作为**符号引用**（Symbolic Reference）保存在 Class 文件的常量池中。比如：描述一个方法调用了另外的其他方法时，就是通过常量池中指向方法的符号引用来表示的，那么**动态链接的作用就是为了将这些符号引用转换为调用方法的直接引用**。
...
- \-XX:NewRatio 年轻代（包括 eden 区 + 2个 Survivor 区）与老年代的比值，默认 1:2，即：新生代 ( Young ) = 1/3 的堆空间大小
...
1. 复制过去以后保证空间的连续性，不会出现“碎片”问题
2. 适合新生代大部分都是“朝生夕灭”对象的场景
...
- `-XX:GCTimeRatio` 垃圾收集时间占总时间的比例（=1/（N+1））。用于衡量吞吐量的大小。
  ...
  - 与前一个`-XX:MaxGCPauseMillis `参数有一定矛盾性。GC 暂停时间越长，GCTimeRatio 参数就容易超过设定的比例。
...
- [深入解析CMS垃圾回收器](https://mp.weixin.qq.com/s?__biz=Mzg4Nzc3NjkzOA==&mid=2247486628&idx=1&sn=984b273af7d1d0398517a2f5442ffb38&chksm=cf847f61f8f3f677372a5ebc9f81403a8324be1bed49bf92e763882715c943324de4f1b0139a&cur_album_id=2713703593714941953&scene=189#wechat_redirect)
- [深入理解 JVM 的垃圾收集器：CMS、G1、ZGC](https://javabetter.cn/jvm/gc-collector.html)
...
另外，由于在垃圾收集阶段用户线程没有中断，所以在 CMS 回收过程中，还应该确保应用程序用户线程有足够的内存可用。因此，CMS 收集器不能像其他收集器那样等到老年代几乎完全被填满了再进行收集，而是当堆内存使用率达到某一阈值时，便开始进行回收，以确保应用程序在 CMS 工作过程中依然有足够的空间支持应用程序运行。要是 CMS 运行期间预留的内存无法满足程序需要，就会出现一次“`Concurrent Mode Failure`” 失败，这时虚拟机将启动后备预案：临时启用 Serial Old 收集器来重新进行老年代的垃圾收集，这样停顿时间会很长！
...
- **标记 - 清除算法遗留内存碎片**，往往出现老年代空间剩余，但无法找到足够大连续空间来分配当前对象，不得不提前触发一次 Full GC。
...
- `-XX:CMSFullGCsBeforeCompaction` 设置在执行多少次 Full GC 后对内存空间进行压缩整理，低配置运行环境，比如老年代在 500 MB 左右，建议不设置，即每次 Full GC 后都整理，以确保有连续的空间存放较大地对象。中高配置运行环境，堆内存 2G+，可以适当设置 2~5 次 Full GC 之后再整理。
...
- [深入理解 JVM 的垃圾收集器：CMS、G1、ZGC](https://javabetter.cn/jvm/gc-collector.html)
- [G1垃圾回收器在并发场景调优](https://www.cnblogs.com/javazhishitupu/p/16020818.html)
...
原因就在于应用程序所应对的**业务越来越庞大、复杂，用户越来越多**，没有 GC 就不能保证应用程序正常进行，而经常造成 STW 的 GC 又跟不上实际的需求，所以才会不断地尝试对 GC 进行优化。G1（Garbage-First）垃圾回收器是在 Java 1.7 update 4 之后引入的一个新的垃圾回收器，是当今收集器技术发展的最前沿成果之一。
...
- 使用G1收集器时，它将整个Java堆划分成约 2048 个大小相同的独立Region块，每个Region块大小根据堆空间的实际大小而定，整体被控制在1MB到32MB之间，且为2的N次幂，即1MB，2MB，4MB，8MB，16MB，32MB。可以通过-XX:G1HeapRegionsize设定。所有的Region大小相同，且在JVM生命周期内不会被改变。
...
**筛选回收（Live Data Counting and Evacuation）** ：`STW`，暂停阶段。清理和重置标记状态。用来统计每个`region`中的中被标记为存活对象的数量比例，这个阶段如果发现完全没有活对象的region就会将其整体回收到可分配region列表中。
...
3. 关于**记忆集**和**并发快照**的维护，`G1`也是通过写屏障（`write barrier`）来进行维护。
...
- `-Xms4g -Xmx4g ` 堆内存，对于G1，一般建议2g以上。注意设定 Xms=Xmx，防止发生扩容、缩容。
- `-XX:MetaspaceSize=256m -XX:MaxMetaspaceSize=256m ` 配置元空间初始256m、最大256m。不配置的话，元空间会不受限地占用物理机内存。可在应用启动后，通过 `jstat -gc PID` 查看元空间内存大小是否设置合适，使用率70%左右即可（元空间存储类元信息、方法元信息、常量池，等）

- `-XX:+UseG1GC` 手动指定使用 G1 垃圾收集器执行内存回收任务。

- `-XX:G1HeapRegionSize` 设置每个 Region 的大小。值是 2 的幂，范围是 1MB 到 32MB 之间，目标是根据最小的 Java 堆大小划分出约 2048 个区域。默认是堆内存的 1/2048。

- `-XX:MaxGCPauseMillis` 设置期望达到的最大 GC 停顿时间指标（JVM 会尽力实现，但不保证达到）。默认值是 200ms（人的平均反应速度）。

- `-XX:+ParallelGCThreads` 并行GC线程数，JVM 会根据CPU核数推断，建议默认。

- `-XX:ConcGCThreads` 设置并发标记的线程数，JVM 会根据CPU核数推断。将 n 设置为并行垃圾回收线程数（ParallelGCThreads）的 1/4 左右。

- `-XX:InitiatingHeapOccupancyPercent` 简称为IHOP，设置触发并发GC周期的Java堆占用率阈值。超过此值，就触发GC，默认值是45%。（老年代Region触发混合GC的占比，默认值是45，也就是说老年代占据了堆内存45%的Region的时，会触发混合GC）这个参数和`CMS`一样主要用来防止`Mixed GC`过程中的并发失败，如果过晚进行并发回收，则可能会因为并发过程中剩余的内存不足以满足用户所需要的内存，这就会导致`G1`放弃并发标记，升级为`Full GC`。这种情况一般都能在`GC`中看到`to-space exhausted`字样。

  > 这个参数也不能调的太小，太小会导致过早开始垃圾回收，一直循环，占用`CPU`资源。

- `-XX:+G1UseAdaptiveIHOP` 自动调整IHOP的指，JDK9之后可用。

- `-XX:G1MixedGCCountTarget=8` 并发运行期间，最多经历几次混合收集周期，默认8。这也是为了解决`to-space exhausted`的问题，每次`Mixed GC`多回收一些，老年代空余的内存就会多一些，但是相应的可能会导致暂停时间增加。

- `-XX:ConcGCThreads` 每次`GC`使用的`CPU`数量， 值不是固定。同样也是为了解决`to-space exhausted`的问题，使用线程多，则`GC`便会快一些，代价是用户的`CPU`时间会被占用。

- `-XX:G1ReservePercent=10`: 假天花板数量，默认10，作用是预留`10%`的空间不使用，留给并发周期过程中当可能会出现`to-space exhausted`的问题时候使用，防止出现`to-space exhausted`，预留过多可能会导致内存浪费。

- 不建议设置年轻代大小：不使用`-Xmn`，因为`G1`是通过需要扩展或缩小年轻代大小，如果设置了年轻代大小，则会导致`G1`无法使用暂停时间目标。



#### 调优实战操作

- [G1垃圾回收器在并发场景调优](https://juejin.cn/post/7076309178045693989)

- [JVM实战-G1参数调优](https://www.cnblogs.com/yu007/p/17900844.html)
...
- [ZGC关键技术分析](https://mp.weixin.qq.com/s/W-RDY1mLDQG86hlLMNM0IQ)

- [12 张图带你彻底理解Java新的垃圾收集器ZGC](https://blog.csdn.net/cxyxysam/article/details/136565793)
...
> Survivor区的对象年龄从小到大进行累加，当累加到 X 年龄时的总和大于50%（可以使用-XX:TargetSurvivorRatio=? 来设置保留多少空闲空间，默认值是50），那么比X大的都会晋升到老年代。
...
- [ GC日志分析](https://www.cnblogs.com/shay/p/13159397.html)
- [GC日志,参数,以及内存分配和回收策略](https://juejin.cn/post/6844904031471534088)
...
[Par0ldGen: 416K->5453K ---- GC前后老年代使用量
(50176K) ] ---- 老年代总大小
5520K->5453K ---- GC前后堆内存使用量
 (182272K), ---- 堆空间（年轻代和老年代）总大小
...
```
# https://javaguide.cn/java/jvm/jvm-parameters-intro.html#_3-2-gc-%E6%97%A5%E5%BF%97%E8%AE%B0%E5%BD%95
# 必选
# 打印基本 GC 信息
-XX:+PrintGCDetails
...
JAVA_OPTS="-server -Xms800m -Xmx800m -Xmn480m -XX:MetaspaceSize=300m -XX:MaxMetaspaceSize=300m -XX:CompressedClassSpaceSize=300m -XX:PermSize=300m -XX:MaxPermSize=300m -XX:MaxDirectMemorySize=300m -XX:-UseAdaptiveSizePolicy -XX:SurvivorRatio=4 -XX:TargetSurvivorRatio=90 -XX:+UseConcMarkSweepGC -XX:+UseParNewGC -XX:+UseCMSCompactAtFullCollection -XX:CMSFullGCsBeforeCompaction=2 -XX:+UseCMSInitiatingOccupancyOnly -XX:CMSInitiatingOccupancyFraction=80 -XX:+ExplicitGCInvokesConcurrent -XX:-OmitStackTraceInFastThrow -XX:+PrintCommandLineFlags -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=logs/gc/mypages -XX:+PrintGCDetails -XX:+PrintGCDateStamps -XX:+PrintGCApplicationStoppedTime -XX:+PrintTenuringDistribution -XX:+PrintHeapAtGC -XX:+PrintReferenceGC -Xloggc:logs/gc/mypages/gc-%t.log -XX:+UseGCLogFileRotation -XX:NumberOfGCLogFiles=5 -XX:GCLogFileSize=10M"

# 减少内存 JDK1.8 CMS
# 小内存建议每次 Full GC 后都整理老年代空间，确保有连续空间。不设置 -XX:CMSFullGCsBeforeCompaction=5
